...
| `backend/lib/gokite-aa-sdk.js` | useful v0.7/userOp logic; local address fallback used the wrong initializer shape | new builder after contract strategy is approved |
| `backend/contracts/KTraceAccountV3SessionExecute.sol` | clean tracked source, but broad owner/direct-call behavior requires an explicit strategy decision | G1 Owner gate |
| `backend/contracts/KTraceAccountFactoryV2.sol` | correct two-argument initializer and v0.7 EntryPoint pin | candidate pin for G1 Owner gate |
